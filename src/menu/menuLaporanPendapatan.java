 ...
 * @author user
 */
public class menuLaporanPendapatan extends javax.swing.JPanel {

    /**
     * Creates new form menuBahan
     */
    private int halamanSaatIni = 1;
    private int dataPerHalaman = 200;
    ...
    private void initComponents() {
        ...
        tblData.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID Transaksi", "Tgl Transaksi", "Untung/transaksi", "Petugas"
            }
        ));
        ...
        databahan1.setText("Menu Laporan Pendapatan");
    ...
    try {
            ...
            String reportPath = "src/report/pendapatanLaporan.jasper"; 
    ...
    private int getTotalData(){
        ...
        try {
            String sql = "SELECT COUNT(*) AS total FROM transaksi WHERE tgl_penjualan BETWEEN ? AND ?\n";
    ...
    try {
        ...
        String sql = "SELECT \n" +
                     ...
                     "WHERE t.tgl_penjualan BETWEEN ? AND ?\n" +
        ...
        try (PreparedStatement st = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                String tanggalPenjualan = rs.getString("tgl_penjualan");
                String tanggalHanya = tanggalPenjualan.split(" ")[0];
                String untung = rs.getString("untung_per_transaksi");
                String namaPetugas = rs.getString("petugas");

                Object[] rowData = {idTransaksi, tanggalHanya, untung, namaPetugas};
                model.addRow(rowData);
            }
        }
    } catch (Exception e) {
        Logger.getLogger(menuLaporanPendapatan.class.getName()).log(Level.SEVERE, null, e);
    }
}
    ...
    try {
        String sql = "SELECT \n" +
                     ...
                     "FROM transaksi t\n" +
                     "INNER JOIN user u ON u.id_user = t.id_user\n" +
                     "WHERE t.id_transaksi LIKE ? OR DATE(t.tgl_penjualan) LIKE ? OR t.untung_per_transaksi LIKE ? OR\n" +
                     "u.nama LIKE ?" +
                     "ORDER BY t.id_transaksi ASC";
        try (PreparedStatement st = conn.prepareStatement(sql)) {
            // Set parameter query untuk semua kolom
            st.setString(1, "%" + kataKunci + "%");
            st.setString(2, "%" + kataKunci + "%"); // Ini akan mencocokkan tanggal tanpa jam
            ...
            if (!rs.isBeforeFirst()) { // Cek jika tidak ada hasil
                ...
                while (rs.next()) {
                    ...
                    Object[] rowData = {idTransaksi, tanggalPenjualan.split(" ")[0], untung, namaPetugas}; // Ambil hanya tanggal
                    model.addRow(rowData);
                }
            }
        }
