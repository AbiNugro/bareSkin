...
public class menuMember extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        tPenjualan.setText("MENU MEMBER");
        pnHeader.add(tPenjualan, new org.netbeans.lib.awtextra.AbsoluteConstraints(660, 10, -1, -1));
        ...
        try {
           ...
           try (PreparedStatement st = conn.prepareStatement(sql)) {
               ...
               while (rs.next()) {
                    ...
                    Object[] rowData = {idMember, namaMember, alamat, 
                        noTelp, poin};
    ...
    private void searchData() {
        ...
        try {
            String sql = "SELECT * FROM member WHERE id_member LIKE ? OR nama_member LIKE ? OR alamat LIKE ? OR no_telepon LIKE ? OR CAST(poin AS CHAR) LIKE ?";
            try (PreparedStatement st = conn.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    String noTelp = rs.getString("no_telepon");
                    int poin = rs.getInt("poin");
                    Object[] rowData = {idMember, namaMember, alamat, noTelp,poin};
                    model.addRow(rowData);
                }
            }
        } catch (Exception e) {
    ...
    private void ubahData() {
        ...
        String telp = tblData.getValueAt(row, 3).toString();
        String poin = tblData.getValueAt(row, 4).toString();

        tambahMember form = new tambahMember(null, true, id, nama, alamat, telp, poin);
